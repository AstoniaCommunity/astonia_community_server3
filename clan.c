...
void tick_clan(void) {
    ...
    switch (update_state) {
    case 0:
        if (read_storage(1, version)) update_state++;
        break;
    case 1:
        res = check_read_storage(&version, &tmp, &size);
        if (res == 1) {
            if (size) {
                if (size != sizeof(clan)) {
...
int set_clan_dungeon_use(int cnr, int type, int number, int cn) {
    ...
    case 18:
        ...
        break; // warrior, mage, seyan +0...+5 each
    case 19:
        if (number < 0 || number > 25) return -1;
        break; // teleport traps
    case 20:
    ...
    case 5:
    case 6:
        clan[cnr].dungeon.warrior[1][type - 1] = number;
        break;
    case 7:
    case 8:
    case 9:
    ...
    case 11:
    case 12:
        clan[cnr].dungeon.mage[1][type - 7] = number;
        break;
    case 13:
    case 14:
    case 15:
    ...
    case 17:
    case 18:
        clan[cnr].dungeon.seyan[1][type - 13] = number;
        break;
    case 19:
        clan[cnr].dungeon.teleport[1] = number;
        break;
    case 20:
        clan[cnr].dungeon.fake[1] = number;
        break;
    case 21:
        clan[cnr].dungeon.key[1] = number;
        break;
    }
    clan_changed = 1;
...
int get_clan_dungeon(int cnr, int type) {
    ...
    switch (type) {
    case 1:
        return clan[cnr].dungeon.warrior[1][0];
    case 2:
        return clan[cnr].dungeon.warrior[1][1];
    case 3:
        return clan[cnr].dungeon.warrior[1][2];
    case 4:
        return clan[cnr].dungeon.warrior[1][3];
    case 5:
        return clan[cnr].dungeon.warrior[1][4];
    case 6:
        return clan[cnr].dungeon.warrior[1][5];

    case 7:
        return clan[cnr].dungeon.mage[1][0];
    case 8:
        return clan[cnr].dungeon.mage[1][1];
    case 9:
        return clan[cnr].dungeon.mage[1][2];
    case 10:
        return clan[cnr].dungeon.mage[1][3];
    case 11:
        return clan[cnr].dungeon.mage[1][4];
    case 12:
        return clan[cnr].dungeon.mage[1][5];

    case 13:
        return clan[cnr].dungeon.seyan[1][0];
    case 14:
        return clan[cnr].dungeon.seyan[1][1];
    case 15:
        return clan[cnr].dungeon.seyan[1][2];
    case 16:
        return clan[cnr].dungeon.seyan[1][3];
    case 17:
        return clan[cnr].dungeon.seyan[1][4];
    case 18:
        return clan[cnr].dungeon.seyan[1][5];

    case 19:
        return clan[cnr].dungeon.teleport[1];
    case 20:
        return clan[cnr].dungeon.fake[1];
    case 21:
        return clan[cnr].dungeon.key[1];
    default:
        return 0;
    }
}
...
static void reduce_clan_bonus(int cnr) {
    ...
    for (n = 0; n < MAXBONUS; n++) {
        if (clan[cnr].bonus[n].level > bestl) {
            bestl = clan[cnr].bonus[n].level;
            bestn = n;
        }
    }
    if (bestl) {
...
void clan_dungeon_chat(char *ptr) {
    ...
    switch (type) {
    case 'T':
        if (clan[cnr].dungeon.teleport[0] > 0) {
            clan[cnr].dungeon.teleport[0]--;
            clan_changed = 1;
        }
        break;
    case 'F':
        if (clan[cnr].dungeon.fake[0] > 0) {
            clan[cnr].dungeon.fake[0]--;
            clan_changed = 1;
        }
        break;
    case 'K':
        if (clan[cnr].dungeon.key[0] > 0) {
...
void show_clan_pots(int cn) {
    ...
    if (!(cnr = get_char_clan(cn))) {
        log_char(cn, LOG_SYSTEM, 0, "Only for clan members.");
        return;
    }
    if (ch[cn].clan_rank < 1) {
        log_char(cn, LOG_SYSTEM, 0, "Not of sufficient rank.");
        return;
    }
