...
int find_login(char *name, char *password, int *area_ptr, int *cn_ptr, int *mirror_ptr, int *ID_ptr, int vendor, int *punique, unsigned int ip) {
    ...
    if (login.status == LS_EMPTY) { // no login waiting? add this one
        ...
        login.age = ticker;
        login.vendor = vendor;
        if (punique) login.unique = *punique;
        else login.unique = 1;
...
void db_read_notes(char *suID, char *srID) {
    ...
    while ((row = mysql_fetch_row(result)) && row[0] && row[1] && row[2]) {
        kind = atoi(row[0]);
        if (row[3]) date = atoi(row[3]);
        else date = 0;

        switch (kind) {
        case 1:
            list_punishment(rID, (void *)row[1], atoi(row[2]), date, atoi(row[4]));
            break;
        }
    }
...
void db_karmalog(char *xrID) {
    ...
    while ((row = mysql_fetch_row(result)) && row[0] && row[1] && row[2]) {
        ...
        switch (kind) {
        case 1:
            karmalog_s(rID, (void *)row[1], atoi(row[2]), date, atoi(row[4]), atoi(row[5]));
            break;
        }
    }
    tell_chat(0, rID, 1, "---");
