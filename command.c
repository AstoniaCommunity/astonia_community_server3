...
int underwater(int cn) {
    ...
    if (map[m].flags&MF_UNDERWATER) {
        for (n=0; n<30; n++) if ((in=ch[cn].item[n]) && in>0 && in<MAXITEM && it[in].driver==IDR_UWTALK) return 0;
...
int command(int cn,char *ptr) {   // 1=ok, 0=repeat
    ...
    if ((len=cmdcmp(ptr,"holler",0))) {
        ptr+=len; while (isspace(*ptr)) ptr++;
        if (ch[cn].flags&CF_SHUTUP) log_char(cn,LOG_SYSTEM,0,"Sorry, you cannot say anything right now.");
    ...
    if (cmdcmp(ptr,"nofireball",5)) {
        if ((ppd=set_data(cn,DRD_LOSTCON_PPD,sizeof(struct lostcon_ppd)))) {
            if (ppd->nofireball) ppd->nofireball=0;
            else ppd->nofireball=1;
            show_lostconppd(cn,ppd);
        }
        return 1;
    }
    ...
    if (cmdcmp(ptr,"nofreeze",5)) {
        if ((ppd=set_data(cn,DRD_LOSTCON_PPD,sizeof(struct lostcon_ppd)))) {
            if (ppd->nofreeze) ppd->nofreeze=0;
            else ppd->nofreeze=1;
            show_lostconppd(cn,ppd);
        }
        return 1;
    ...
    if (cmdcmp(ptr,"autopulse",5)) {
        if ((ppd=set_data(cn,DRD_LOSTCON_PPD,sizeof(struct lostcon_ppd)))) {
            if (ppd->autopulse) ppd->autopulse=0;
            else ppd->autopulse=1;
            show_lostconppd(cn,ppd);
        }
        return 1;
    }
